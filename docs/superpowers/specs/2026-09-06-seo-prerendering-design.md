...
**Update (post-implementation):** during Task 6's verification, `renderToString` was found to be incapable of resolving `Suspense`/`lazy` boundaries at all — it was causing the client to reject the prerendered HTML outright on these four routes (not merely prerender a spinner). This was fixed by switching `entry-server.tsx` to `renderToPipeableStream`'s `onAllReady` callback, which correctly waits for lazy imports to resolve. All four routes below now prerender full real content, not a spinner shell — a strict improvement over the design below, achieved without changing the client-side lazy-loading/bundle-splitting decision.
